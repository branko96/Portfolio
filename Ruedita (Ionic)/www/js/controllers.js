...
.controller('AboutCtrl', function($scope,$state,$rootScope, appConfig, AppService){

})

.controller('HomeCtrl', function($scope, $state, $rootScope, ProductService, appConfig, AppService, listLanguage, appLanguage, LanguageService) {
...
.controller('CheckoutPaymentCtrl', function($scope, $state, $ionicHistory, $ionicPopup, $rootScope, OrderService, UserService, CartService, appConfig, $injector, appValue) {
	...
	$scope.createOrder = function() {
		...
		if(appConfig.ENABLE_MERCADOPAGO_PLUGIN === true && $scope.orderInfo.payment === 'mercadopago' && appValue.API_URL == '/is-commerce/api/') {
			try {
				var MercadoPagoService = $injector.get('MercadoPagoService');
...
.controller('ContactUsCtrl', function($scope, $state, $ionicModal, appConfig, AppService) {
	...
	$scope.callPhoneNum = function(number) {
